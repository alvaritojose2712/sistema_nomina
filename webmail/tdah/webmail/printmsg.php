...
ini_set("display_errors", 1);
	...
	$body = preg_replace("#<body#i","<uebimiau_body_not_alowed",$body);
	...
	$ARFrom[0]["link"] = "newmsg.php?nameto=".urlencode($name)."&mailto=$thismail&lid=$lid&tid=$tid";
	...
	for($i=0;$i<count($ARTo);$i++) {
		...
		$link = "newmsg.php?nameto=".urlencode($name)."&mailto=$thismail&lid=$lid&tid=$tid";
		$ARTo[$i]["link"] = $link;
	...
	if(count($anexos) > 0) {
		for($i=0;$i<count($anexos);$i++) {
			$link1 = "download.php?folder=$folder&ix=$ix&mnum=$mnum&bound=".base64_encode($anexos[$i]["boundary"])."&part=".$anexos[$i]["part"]."&lid=$lid&tid=$tid";
			$link2 = "$link1&down=1";
