...
class UebiMiau extends UebiMiau_core {
	...
	function mail_send_command($cmd) {
		...
		if($this->mail_connected()) {
			$output = (preg_match("/^(PASS|LOGIN)/i",$cmd,$regs))?$regs[1]." ****":$cmd;
	...
	function mail_connect() {
		...
		if(!$this->mail_connected()) {
			...
			if($this->mail_connection) {
				...
				if(preg_match('/'.$regexp.'/',$buffer)) return 1;
				else return 0;
			}
			return 0;
		} else return 1;
	...
	function mail_retr_msg($msg, $check=1) {
		...
		if($this->mail_protocol == "imap") {
			...
			if($check) {
				...
				if(preg_match("/^(".$this->_sid." (NO|BAD))/",$buffer)) { $this->mail_error_msg = $buffer; return 0; }
			...
			if(file_exists($msg["localname"])) {
				...
				$buffer = $this->mail_get_line();
				while(!preg_match("/^(".$this->_sid." OK)/i",$buffer)) {
					if(!preg_match("/[ ]?\\*[ ]?[0-9]+[ ]?FETCH/",$buffer))
						$msgbody .= $buffer;
					$buffer = $this->mail_get_line();
				}
				$pos = strrpos($msgbody, ")");
...
function mail_spam_msg($msg, $send_to_spam = 1, $save_only_read = 0) {
		...
		if($this->mail_protocol == "imap") {
			...
			if( $send_to_spam && 
				strtoupper($msg["folder"]) != "SPAM" &&
				(!$save_only_read || ($save_only_read && $read))) {

				$spam_folder = $this->fix_prefix($this->_sysmap["spam"],1);

				$this->mail_send_command("COPY ".$msg["id"].":".$msg["id"]." \"$spam_folder\"".$this->CRLF);
				$buffer = $this->mail_get_line();
				...
				if(file_exists($msg["localname"])) {
					$currentname = $msg["localname"];
					$basename = basename($currentname);
					$newfilename = $this->user_folder.$this->_sysmap["spam"]."/$basename";
					copy($currentname,$newfilename);
					unlink($currentname);
				}
			}
	...
	function mail_list_msgs($boxname = "INBOX") {
		...
		if($this->mail_protocol == "imap") {
				...
				if(!preg_match("/^(".$this->_sid." (NO|BAD))/i",$buffer)) { 
					...
					while(!preg_match("/^(".$this->_sid." OK)/i",$buffer)) {
						...
						if(preg_match("/[ ]?\\*[ ]?([0-9]+)[ ]?FETCH/i",$buffer,$regs)) {
							$curmsg	= $regs[1];
							preg_match("/SIZE[ ]?([0-9]+)/",$buffer,$regs);
							$size	= $regs[1];
							preg_match("/FLAGS[ ]?\\((.*)\\)/i",$buffer,$regs);
							$flags 	= $regs[1];
						/* if any problem, add the current line to buffer */
						} elseif(trim($buffer) != ")" && trim($buffer) != "" && !preg_match("/[ ]?\\*[ ]?NO/i",$buffer)) {
							$header .= $buffer;
			...
			*/
			if(strtoupper($boxname) == "INBOX") {
				...
				if(!preg_match("/^(\+OK)/",$buffer))  { $this->mail_error_msg = $buffer; return $messages; }
		...
		*/
		...
		for($i=0;$i<count($messages);$i++) {
            ...
            $messages[$i]["attach"] = (preg_match("#(multipart/mixed|multipart/related|application)#",$mail_info["content-type"]))?1:0;
	...
	function mail_list_boxes($boxname = "*") {
		...
		if($this->mail_protocol == "imap") {
			...
			while(!preg_match("/^(".$this->_sid." OK)/i",$buffer)) {
				...
				$tmp["name"] = mb_convert_encoding( $tmp["name"], "ISO_8859-1", "UTF7-IMAP" ); 	// Fix for Outlook IMAP encoding names
				$buffer = $this->mail_get_line();
				$boxlist[] = $tmp;
			}
		} else {
	...
	function mail_save_message($boxname,$message,$flags = "") {
		if($this->mail_protocol == "imap") {
			...
			if(!preg_match("/^(".$this->_sid." OK)/i",$buffer)) return 0; 
		}
